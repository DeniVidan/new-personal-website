        ...
        {steps.map((step, index) => (
          <>
            <div 
              ...
              className="font-raleway px-8 py-4 transition-all duration-300 relative space-y-10"
            >
              <div className="relative w-full h-10 md:h-14 top-4 left-0">
                <div className="font-custom absolute top-0 left-0 bg-white w-12 h-12 md:w-16 md:h-16 flex items-center justify-center rounded text-black text-2xl md:text-4xl font-medium">
                  {step.number}
                </div>
              </div>
              <p className="text-gray-400 mt-8 md:text-2xl text-center">{"[ "+step.secondaryText+" ]"}</p>
              <h2 className="text-3xl md:text-6xl font-black text-center">{step.title}</h2>
              <p className="text-lg md:text-3xl text-gray-300 text-center">{step.mainText}</p>
            </div>
